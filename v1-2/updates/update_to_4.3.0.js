...
async function getInvoices() {
    const find = `SELECT commission_id, invoice_id FROM invoice`;
...
async function updateView() {
    const update = `CREATE OR REPLACE VIEW "complete_commission" AS
    ...
    FROM invoice
    WHERE invoice.invoice_id = co.invoice_id)
    AS "invNumber",
         co.notification,
         co."orderDate",
         co."orderNumber"
...
async function run() {
    let update = `ALTER TABLE "commission"
    ...
    try {
        ...
        for await (const { commission_id: commission, invoice_id: invoice } of invoices) {
            const update = `UPDATE "commission" SET "invoice_id" = '${invoice}' where commission_id = '${commission}'`;

            await pool.query(update);
            console.warn(commission, 'updated');
        }
    } catch (e) {
        console.error(e);
