...
describe('/api/loading', () => {
  describe(`/api/${endpointName}`, () => {
    beforeEach(async () => {
      ...
      let query = `INSERT INTO "loading" ("customer_id", "email", "firstName", "lastName", "phone") values ('18', 'miroslav.sirina@koala42.com', 'Marek', 'Řebíček', '603 574 374') RETURNING "loading_id"`;
    ...
    describe('REMOVE /:id', () => {
      it('should delete the loading and return it', async () => {
        let res = await request(server)
          .delete(`/api/loading/${loadingId}`)
          .set('x-auth-token', token);

        expect(res.status).toBe(200);
        expect(res.body).toHaveProperty('loading_id');
