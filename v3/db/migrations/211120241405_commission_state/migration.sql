CREATE TYPE public."CommissionState" AS ENUM ('ENQUIRY', 'INCOMPLETE', 'COMPLETE');

ALTER TYPE public."CommissionState" OWNER TO postgres;

ALTER TABLE
    IF EXISTS public.commission
ADD
    COLUMN "stateTemp" "CommissionState" NOT NULL DEFAULT 'INCOMPLETE';
...
SET
    "stateTemp" = CASE
        WHEN state = 0 THEN 'INCOMPLETE' :: "CommissionState"
        WHEN state = 1 THEN 'COMPLETE' :: "CommissionState"
        ELSE 'INCOMPLETE' :: "CommissionState"
    END;

ALTER TABLE
    IF EXISTS public.commission DROP COLUMN "state";

ALTER TABLE
    IF EXISTS public.commission
ADD
    COLUMN "state" "CommissionState" NOT NULL DEFAULT 'INCOMPLETE';

UPDATE
    public.commission
SET
    state = "stateTemp" :: "CommissionState";

ALTER TABLE
    public.commission DROP COLUMN "stateTemp";

CREATE
OR REPLACE VIEW public.complete_commission AS
SELECT
    co.commission_id,
    co.relation,
    co.invoice_id,
    ...
    CASE
        ...
        WHEN co."exchangeRateCarrier" IS NOT NULL
        AND co."currencyCarrier" = 'CZK' :: text THEN co."priceCarrier"
    ...
    CASE
        ...
        WHEN co."exchangeRateCustomer" IS NOT NULL
        AND co."currencyCustomer" = 'CZK' :: text THEN co."priceCustomer"
        ELSE co."exchangeRateCustomer" * co."priceCustomer"
    END AS "priceCustomer",
    CASE
        WHEN co."priceCarrier" IS NULL
        AND co."priceCustomer" IS NULL THEN NULL :: numeric
        ELSE count_provison(
            co."exchangeRateCarrier",
            co."priceCarrier",
            co."currencyCarrier",
    ...
    (
        SELECT
            array_agg(location.city) AS array_agg
        FROM
            location
        WHERE
            location.location_id = ANY (cl.loading_ids)
    ) AS loading_city,
    (
        SELECT
            array_agg(location."postalCode") AS array_agg
        FROM
            location
        WHERE
            location.location_id = ANY (cl.loading_ids)
    ...
    (
        SELECT
            string_agg(location.city, '; ' :: text) AS string_agg
        FROM
            location
        WHERE
            location.location_id = ANY (cl.loading_ids)
    ...
    (
        SELECT
            string_agg(location."postalCode", '; ' :: text) AS string_agg
        FROM
            location
        WHERE
            location.location_id = ANY (cl.loading_ids)
    ...
    (
        ...
        FROM
            location
        WHERE
            location.location_id = ANY (cd.discharge_ids)
    ) AS discharge_city,
    (
        SELECT
            array_agg(location."postalCode") AS array_agg
...
FROM
    ...
    LEFT JOIN (
        SELECT
            "Attachment".commission_id,
            "Attachment".attachment_id,
            "Attachment".name AS "carrierInvoiceName"
        FROM
            "Attachment"
        WHERE
            "Attachment".type = 'INVOICE' :: "AttachmentType"
            AND "Attachment".deleted = false
    ) at ON at.commission_id = co.commission_id
    LEFT JOIN (
        SELECT
...
ALTER TABLE
    public.complete_commission OWNER TO postgres;
